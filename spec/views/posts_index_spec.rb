...
RSpec.describe Post, type: :model do
  describe 'For the Post model' do
    before(:each) do
      ...
      @post = Post.create(user: @user, title: 'Tests', text: 'testing', likes_counter: 7,
                          comments_counter: 5)
    end
    before { @post.save }
...
RSpec.describe 'Posts show page', type: :feature do
  before(:each) do
    User.destroy_all
    @user = User.create(name: 'Faith', photo: 'picture.png',
                        bio: 'Tech sis from Nigeria',
                        email: 'faithy@mail.com', password: 'password',
                        confirmed_at: Time.now, role: 'admin', posts_counter: 0)

    Post.create(title: 'My title', text: 'My text',
                user_id: @user.id, likes_counter: 0, comments_counter: 0)
    ...
    fill_in 'Email', with: 'faithy@mail.com'
